...
async def main_MemberCounterMeta():
    async with MemberCounterMeta:
        try:
            while True:
                print(text_2)
                edit_message_text_teletips = "**📈 | Emo Real-Time Member Counter** [ᵐᵉᵗᵃ](https://github.com/RishBropromax/Emo-Member-Counter-Bot#:~:text=Versions-,1.0%20Meta,-Credits)"
                for CHANNEL_OR_GROUP in CHANNEL_OR_GROUP_LIST:
                    try:
                        get_chat_teletips = await Emo MemberCounterMeta.get_chat(int(CHANNEL_OR_GROUP))   
...
async def alive(_, message: Message):
    await message.edit("Your Emo MemberCounter is alive!")
